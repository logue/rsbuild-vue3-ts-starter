...
2. Run mandatory checks in this order:
   - `pnpm lint` (must run first)
   - `pnpm type-check`
3. Run targeted tests based on impact:
   - component/store logic changes: `pnpm test`
   - route/view flow changes: `pnpm test:e2e` (when available)
   - if `test:e2e` is not available yet, explicitly report it as "planned/skipped"
